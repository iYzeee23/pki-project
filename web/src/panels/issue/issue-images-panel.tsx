...
import { issueImagesTexts } from "../../i18n/i18n-builder";

export function IssueImagesPanel() {
  const { t } = useTranslation();
  const iip = issueImagesTexts(t);
  ...
  return (
    <Panel title={iip.IssueImages} onClose={closeTo}>
      {busy ? <div style={{ color: "#888" }}>{iip.Loading}</div> : null}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {!busy && items.length === 0 ? (
          <div style={{ color: "#888", fontSize: 14 }}>{iip.NoImages}</div>
        ) : null}
